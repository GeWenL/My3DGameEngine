...
#include "T3DLIB1.H"    // Math is based on some defs in this 
#include "Math.H"
#include "MVPTrans.H"
...
// compute average and max radius 求平均半径和最大半径,供3D流水线的后面进行碰撞检测和物体消除时使用
	...
	// Step 5: load the polygon list 加载多边形列表
	...
	MATRIX4X4_PTR mrot) // output 
{
	...
	/*float scale = 1.01;
	float tranX = 0.5;
	Mat_Init_4X4(mrot, scale, 0, 0, 0,
		0, scale, 0, 0,
		0, 0, scale, 0,
		tranX, 0, 0, 1);*/

	//Mat_Mul_4X4(&my, &scaleMt, mrot);
...
void Transform_OBJECT4DV1(OBJECT4DV1_PTR obj, // object to transform 要变换的物体
	MATRIX4X4_PTR mt,   // transformation matrix 变换矩阵
	int coord_select,   // selects coords to transform 指定对哪种坐标进行变换
	int transform_basis) // flags if vector orientation 指定是否要对朝向向量进行变换
						 // should be transformed too
{
	...
	if (transform_basis)
	{
		...
		// 旋转物体的朝向向量
		VECTOR4D vresult; // use to rotate each orientation vector axis 用于暂时存储旋转结果

		// rotate ux of basis 旋转ux
		Mat_Mul_VECTOR4D_4X4(&obj->ux, mt, &vresult);// 对点进行变换
		VECTOR4D_COPY(&obj->ux, &vresult);//将结构存回去

		// rotate uy of basis 旋转uy
		Mat_Mul_VECTOR4D_4X4(&obj->uy, mt, &vresult);
		VECTOR4D_COPY(&obj->uy, &vresult);

		// rotate uz of basis 旋转uz
...
void Camera_To_Perspective_Screen_OBJECT4DV1(OBJECT4DV1_PTR obj, CAM4DV1_PTR cam)
{
	...
	for (int vertex = 0; vertex < obj->num_vertices; vertex++)
	{
		...
		// transform:
		obj->vlist_trans[vertex].x = obj->vlist_trans[vertex].x * alpha + alpha;
		obj->vlist_trans[vertex].y = -obj->vlist_trans[vertex].y * beta + beta;
